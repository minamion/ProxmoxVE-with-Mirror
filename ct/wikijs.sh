...
function update_script() {
    ...
    RELEASE=$(curl -fsSL https://proxy.seaslug.moe/api.github.com/repos/Requarks/wiki/releases/latest | grep "tag_name" | awk '{print substr($2, 3, length($2)-4) }')
    if [[ "${RELEASE}" != "$(cat /opt/${APP}_version.txt)" ]] || [[ ! -f /opt/${APP}_version.txt ]]; then
        ...
        rm -rf /opt/wikijs/*
        cd /opt/wikijs
        curl -fsSL "https://proxy.seaslug.moe/github.com/requarks/wiki/releases/download/v${RELEASE}/wiki-js.tar.gz" -o $(basename "https://github.com/requarks/wiki/releases/download/v${RELEASE}/wiki-js.tar.gz")
