...
function update_script() {
    ...
    RELEASE=$(curl -fsSL https://proxy.seaslug.moe/api.github.com/repos/jordan-dalby/ByteStash/releases/latest | grep "tag_name" | awk '{print substr($2, 3, length($2)-4) }')
    if [[ ! -f /opt/${APP}_version.txt ]] || [[ "${RELEASE}" != "$(cat /opt/${APP}_version.txt)" ]]; then
        ...
        temp_file=$(mktemp)
        curl -fsSL "https://proxy.seaslug.moe/github.com/jordan-dalby/ByteStash/archive/refs/tags/v${RELEASE}.tar.gz" -o "$temp_file"
        tar zxf $temp_file
