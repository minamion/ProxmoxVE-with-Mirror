...
function update_script() {
  ...
  RELEASE=$(curl -fsSL https://proxy.seaslug.moe/api.github.com/repos/CrazyWolf13/streamlink-webui/releases/latest | grep "tag_name" | awk '{print substr($2, 2, length($2)-3) }')
