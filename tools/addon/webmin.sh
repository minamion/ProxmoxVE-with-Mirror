...
LATEST=$(curl -fsSL https://proxy.seaslug.moe/api.github.com/repos/webmin/webmin/releases/latest | grep '"tag_name":' | cut -d'"' -f4)

msg_info "Downloading Webmin"
curl -fsSL "https://proxy.seaslug.moe/github.com/webmin/webmin/releases/download/$LATEST/webmin_${LATEST}_all.deb" -o $(basename "https://github.com/webmin/webmin/releases/download/$LATEST/webmin_${LATEST}_all.deb")
