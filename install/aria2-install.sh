...
if [[ ${prompt,,} =~ ^(y|yes)$ ]]; then
  ...
  curl -fsSL "$(curl -fsSL https://proxy.seaslug.moe/api.github.com/repos/mayswind/ariang/releases/latest | grep download | grep AllInOne.zip | cut -d\" -f4)" -o $(basename "$(curl -fsSL https://proxy.seaslug.moe/api.github.com/repos/mayswind/ariang/releases/latest | grep download | grep AllInOne.zip | cut -d\" -f4)")
