...
RELEASE_SERVER=$(curl -fsSL https://proxy.seaslug.moe/api.github.com/repos/apache/guacamole-server/tags | jq -r '.[].name' | grep -v -- '-RC' | head -n 1)
...
$STD make install
$STD ldconfig
RELEASE_CLIENT=$(curl -fsSL https://proxy.seaslug.moe/api.github.com/repos/apache/guacamole-client/tags | jq -r '.[].name' | grep -v -- '-RC' | head -n 1)
