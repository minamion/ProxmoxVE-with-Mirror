...
RELEASE_SERVER=$(curl -fsSL https://proxy.seaslug.moe/api.github.com/repos/apache/guacamole-server/tags | jq -r '.[0].name')
...
$STD make install
$STD ldconfig
RELEASE_CLIENT=$(curl -fsSL https://proxy.seaslug.moe/api.github.com/repos/apache/guacamole-client/tags | jq -r '.[0].name')
