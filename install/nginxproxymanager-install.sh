...
$STD bash <(curl -fsSL https://proxy.seaslug.moe/raw.githubusercontent.comnvm-sh/nvm/v0.39.7/install.sh)
source ~/.bashrc
...
RELEASE=$(curl -fsSL https://proxy.seaslug.moe/api.github.com/repos/NginxProxyManager/nginx-proxy-manager/releases/latest |
  grep "tag_name" |
